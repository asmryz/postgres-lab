...
function App() {
	...
	const [schema, setSchema] = useStickyState("", "schema");

	// useEffect(() => {
	//     setSchema(window.localStorage.getItem("schema")|| "public");
	// }, []);

	useEffect(() => {
		if (schema !== "") {
			fetch(`/schema/${schema}`).then((res) => res.json());
		}
	}, [schema]);
				...
				<>
					...
					<div style={{ marginTop: "90px", height: "calc(100vh - 80px)", overflowY: "auto", width: "4000px" }}>
						{Array.isArray(result) && result.length > 0 ? (
											...
											<tr>
												{index === 0 &&
													Object.keys(item).map((key, idx) => (
														<th key={idx} style={{ fontWeight: "bolder" }}>
															{key}
														</th>
													))}
											</tr>
							...
							<div style={{ padding: "1rem", color: "#888" }}>{typeof result === "string" ? result : "No results or result is not an array."}</div>
						)}
					</div>
				</>
			)}
			<pre style={{ textAlign: "left" }}>{JSON.stringify({ schema }, null, 4)}</pre>
		</>
	);
}
